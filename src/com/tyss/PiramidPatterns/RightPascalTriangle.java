...
public class RightPascalTriangle {
  ...
  // Logic 1
		...
		for (int i = (-n) + 1; i < n; i++) {
			...
			for (int j = n - 1; j >= 0; j--) {
				if (a <= j )
					/* ---> "If you want "Hollow  Inverted Right Pascal Triangle",' just replace a <= j with a >= j. The remaining code stays the same; do not change anything else."
 ...
 * for(int i=1;i<=n*2-1;i++) {
			...
			for(int j=1;j<=n;j++) {
				if( i>=j && i+j<=n*2 ) 
				---> "If you want "Hollow  Inverted Right Pascal Triangle",' just replace if( i>=j && i+j<=n*2 )  with if( i<=j || i+j>=n*2 ). The remaining code stays the same; do not change anything else."
