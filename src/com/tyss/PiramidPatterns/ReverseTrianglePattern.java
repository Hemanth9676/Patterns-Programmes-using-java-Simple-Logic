		...
		for (int i = n - 1; i >= 0; i--) {
			for (int j = (-n) + 1; j < n; j++) {
				...
				if (a < 0)
					a = -a;
				if (i >= a)
					/* ---> "If you want "Hollow Reverse Inverted Pyramid Star Pattern",' just replace i >= a with i <= a. The remaining code stays the same; do not change anything else."
 ...
 * for (int i = 1; i <= n; i++) {
			for (int j = 1; j < n * 2; j++) {
				if (i <= j && i + j <= n * 2) {
				---> "If you want "Hollow Reverse Inverted Pyramid Star Pattern",' just replace if (i <= j && i + j <= n * 2) with if (i >= j || i + j >= n * 2). The remaining code stays the same; do not change anything else."
